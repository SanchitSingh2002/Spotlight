...
export const createPost = mutation({
    ...
    handler: async(ctx, args) => {
        ...
        const currenUser =  await getAuthenticatedUser(ctx);
    ...
    handler: async (ctx) => {
        ...
        const postsWithInfo = await Promise.all(posts.map(async (post) => {
            ...
            const like = await ctx.db.query("likes").withIndex("by_user_and_post", (q) =>
                q.eq("userId", currenUser._id).eq("postId", post._id)).first();
...
export const toggleLike = mutation({
    ...
    handler: async(ctx, args) => {
        ...
        // checking whether we've already liked the post or not
        ...
        if(existing){
            ...
            // if it's not my post then create a notification
...
export const deletePost = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // delete associated likes with post
        ...
        // decrement the user posts count by 1
