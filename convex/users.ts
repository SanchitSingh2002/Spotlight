...
export const createUser = mutation({
    args: {
        ...
        const isExistingUser = await ctx.db.query("users")
...
export async function getAuthenticatedUser(ctx: QueryCtx | MutationCtx) {
    ...
    const currenUser = await ctx.db.query("users").withIndex(
...
export const getUsersByClerkId = query({
    ...
    handler: async (ctx, args) => {
        const user = await ctx.db.query("users").withIndex(
            "by_clerk_id", (q) => q.eq("clerkId", args.clerkId)
        ).unique();
    ...
    handler: async (ctx, args) => {
        ...
        const follow = await ctx.db.query("follows")
            .withIndex("by_both", (q) => q.eq("followerId", currentUser._id)
                .eq("followingId", args.followingId)).first();
